...
const LoginWithBackgroundCorrect = () => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [backgroundImage, setBackgroundImage] = useState<string>('/images/background-login.jpg');
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      ...
      const alternativeNames = [
        ...
        '/images/academie-bg.jpg'
      ];
      
      let tryIndex = 0;
      const tryNextImage = () => {
        if (tryIndex < alternativeNames.length) {
          const testImg = new Image();
          testImg.onload = () => {
            setBackgroundImage(alternativeNames[tryIndex]);
            setImageLoaded(true);
          };
          testImg.onerror = () => {
            tryIndex++;
            if (tryIndex < alternativeNames.length) {
              ...
              // Aucune image trouvée, utiliser le fallback
              setImageLoaded(false);
            }
          };
          testImg.src = alternativeNames[tryIndex];
        }
      };
...
export default LoginWithBackgroundCorrect;
